	...
	$scope.FormElements = function() {
		$http({
			...
			if (response.builder.trim() === '') {
				...
				/*
				for (let x in raw_builder.FormElements) {
					for (let y in raw_builder.FormElements[x]) {
						if (raw_builder.FormElements[x][y].elementDefaults.selectedIcon === 'no-icon') {
							raw_builder.FormElements[x][y].elementDefaults.selectedIcon = ''
						} else if ( typeof raw_builder.FormElements[x][y].elementDefaults.selectedIcon !== 'undefined' && raw_builder.FormElements[x][y].elementDefaults.selectedIcon !== '') {
							let thisIcon = raw_builder.FormElements[x][y].elementDefaults.selectedIcon
							thisIcon = thisIcon.replace('icon-', '')
							let newIcon = $scope.listIcons.find((x) => {
								if (thisIcon === x.Value.substring(0, thisIcon.length)) {
									return true
								}
							})
							if (typeof newIcon !== 'undefined') {
								raw_builder.FormElements[x][y].elementDefaults.selectedIcon = newIcon.Value
							}
						}
					}
				}
